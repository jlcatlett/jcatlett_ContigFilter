...
class jcatlett_ContigFilterTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'jcatlett_ContigFilter',
        ...
        for nameval in config.items('jcatlett_ContigFilter'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = jcatlett_ContigFilter(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_jcatlett_ContigFilter_" + str(suffix)
