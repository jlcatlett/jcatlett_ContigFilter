package jcatlett_ContigFilter::jcatlett_ContigFilterClient;
...
jcatlett_ContigFilter::jcatlett_ContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => jcatlett_ContigFilter::jcatlett_ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a jcatlett_ContigFilter.FilterContigsParams
$return is a jcatlett_ContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a jcatlett_ContigFilter.workspace_name
	contigset_id has a value which is a jcatlett_ContigFilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a jcatlett_ContigFilter.ws_contigset_id
...
# Authentication: required
	...
	method => "jcatlett_ContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$params is a jcatlett_ContigFilter.FilterContigsMaxParams
$return is a jcatlett_ContigFilter.FilterContigsResults
...
# Authentication: required
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "jcatlett_ContigFilter.filter_contigs_max",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "jcatlett_ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for jcatlett_ContigFilter::jcatlett_ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "jcatlett_ContigFilter::jcatlett_ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package jcatlett_ContigFilter::jcatlett_ContigFilterClient::RpcClient;
